...
export default function GoalsScreen() {
  ...
  const handleCreateGoal = async () => {
    ...
    if (error) {
      ...
      setNewGoal({ name: '', target_amount: '', emoji: '🎯', color: '#8B5CF6' });
      fetchData();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        ...
        <View style={styles.statCard}>
          <Text style={styles.statValue}>${stats.totalSaved.toLocaleString()}</Text>
      ...
      {loading ? (
        ...
        >
          {goals.length === 0 ? (
            ...
            goals.map((goal) => <GoalCard key={goal.id} goal={goal} onAddMoney={openAddMoneyModal} />)
          )}
        </ScrollView>
      )}
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.fieldGroup}>
              ...
              <View style={styles.emojiSelector}>
                {['🎯', '📱', '🏖️', '🎮', '🚗', '🏠', '💍', '🎓'].map((emoji) => (
                  <TouchableOpacity key={emoji} style={[styles.emojiOption, newGoal.emoji === emoji && { backgroundColor: newGoal.color, borderColor: '#fff' }]} onPress={() => setNewGoal({ ...newGoal, emoji })}>
              ...
              <View style={styles.colorSelector}>
                {goalColors.map((color) => (
                  <TouchableOpacity key={color} style={[styles.colorOption, { backgroundColor: color }, newGoal.color === color && styles.colorSelected]} onPress={() => setNewGoal({ ...newGoal, color })} />
                ))}
              </View>
            </View>
            <View style={styles.fieldGroup}>
              <Text style={styles.inputLabel}>Goal Name</Text>
              <TextInput style={styles.input} placeholder="e.g., New Gaming PC" placeholderTextColor="#9CA3AF" value={newGoal.name} onChangeText={(name) => setNewGoal({ ...newGoal, name })} />
            </View>
            <View style={styles.fieldGroup}>
              <Text style={styles.inputLabel}>Target Amount</Text>
              <TextInput style={styles.input} placeholder="e.g., 2500" placeholderTextColor="#9CA3AF" value={newGoal.target_amount} onChangeText={(target) => setNewGoal({ ...newGoal, target_amount: target })} keyboardType="numeric" />
            </View>
...
function GoalCard({ goal, onAddMoney }: { goal: Goal, onAddMoney: (goal: Goal) => void }) {
    ...
    <View style={[styles.goalCard, { borderLeftColor: goal.color || '#8B5CF6' }]}>
      ...
      <View style={styles.goalProgress}>
        ...
        <View style={styles.progressBar}>
          <LinearGradient colors={[goal.color || '#8B5CF6', (goal.color || '#8B5CF6') + '80']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[styles.progressFill, { width: `${Math.min(progress, 100)}%` }]} />
        </View>
      ...
      <View style={styles.goalActions}>
        <TouchableOpacity style={styles.actionButton} onPress={() => onAddMoney(goal)}>
...
function AddMoneyModal({ visible, onClose, goal, wallets, onSuccess }: { visible: boolean, onClose: () => void, goal: Goal, wallets: Wallet[], onSuccess: () => void }) {
  const { user } = useAuth();
  ...
  const handleAddMoney = async () => {
    ...
    const [goalUpdate, walletUpdate, txInsert] = await Promise.all([
      supabase.from('goals').update({ current_amount: newGoalAmount }).eq('id', goal.id),
      supabase.from('wallets').update({ balance: newWalletBalance }).eq('id', selectedWalletId),
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <View style={styles.fieldGroup}>
            ...
            <TextInput style={styles.input} placeholder="e.g., 50" placeholderTextColor="#9CA3AF" value={amount} onChangeText={setAmount} keyboardType="numeric" />
          </View>
          <View style={styles.fieldGroup}>
            <Text style={styles.inputLabel}>From Wallet</Text>
            <View style={styles.walletSelector}>
              {wallets.map(w => (
                <TouchableOpacity key={w.id} style={[styles.walletOption, selectedWalletId === w.id && styles.walletSelected]} onPress={() => setSelectedWalletId(w.id)}>
                  <Text style={styles.walletOptionText}>{w.name}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          <TouchableOpacity style={styles.createButton} onPress={handleAddMoney}>
...
const styles = StyleSheet.create({
  ...
  emptyStateTitle: { fontSize: 20, fontWeight: 'bold', color: '#ffffff', marginTop: 16 },
  emptyStateSubtitle: { fontSize: 14, color: '#9CA3AF', marginTop: 8, textAlign: 'center', maxWidth: '70%' },
  goalCard: { backgroundColor: '#1E293B', borderRadius: 16, padding: 20, marginBottom: 16, borderLeftWidth: 4 },
  ...
  addMoneyButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#374151', alignItems: 'center', justifyContent: 'center' },
  ...
  progressBar: { height: 8, backgroundColor: '#374151', borderRadius: 4, marginBottom: 8, overflow: 'hidden' },
  ...
  goalActions: { flexDirection: 'row', gap: 12, borderTopWidth: 1, borderTopColor: '#374151', paddingTop: 16, marginTop: 16 },
  actionButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#374151', paddingVertical: 12, borderRadius: 8, gap: 6 },
  ...
  emojiSelector: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 12 },
  emojiOption: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#374151', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'transparent' },
  ...
  input: { backgroundColor: '#374151', borderRadius: 12, padding: 16, color: '#ffffff', fontSize: 16 },
  ...
  walletOption: { paddingVertical: 10, paddingHorizontal: 16, backgroundColor: '#374151', borderRadius: 20 },
